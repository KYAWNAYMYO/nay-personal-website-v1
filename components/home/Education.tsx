        ...
        >
            ...
            <div className="rounded-2xl border border-brand-50 bg-brand-50/50 p-6 md:p-8">
                ...
                <ul className="mt-6 space-y-3 text-brand-ink/80">
                    {education.modules.map((modules) => (
                        <li key={modules} className="text-base leading-7">
                            {modules}
                        </li>
                    ))}
                </ul>
            ...
            <div className="rounded-2xl border border-brand-50 bg-brand-50/50 p-6 md:p-8">
                ...
                <p className="mt-1 text-brand-ink/70">{education.program2}</p>
                <p className="mt-2 text-sm font-medium uppercase tracking-[0.3em] text-brand-ink/60">
                    {education.start2} – {education.end2}
                </p>
                <ul className="mt-6 space-y-3 text-brand-ink/80">
                    {education.modules2.map((modules) => (
                        <li key={modules} className="text-base leading-7">
                            {modules}
                        </li>
                    ))}
